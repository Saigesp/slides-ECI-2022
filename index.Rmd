...
Podemos también **eliminar los registros ausentes** en alguna de sus variables con `drop_na()`.
...
* 📝 Filtra el conjunto de personajes y quédate solo con aquellos que en la variable `height` no tengan un dato ausente.

* 📝 Después selecciona solo las variables `name`, `height` y todas aquellas variables que CONTENGAN la palabra `color` en su nombre.

* 📝 Del conjunto resultante comprueba que modalidades (y cuántos registros hay en cada una) de la variable `hair_color`

* 📝 Después selecciona solo aquellos registros con el pelo (`hair_color`) negro

* 📝 Del conjunto resultante, ordena los personajes de menor a mayor altura (`height`).

* 📝 Por último renombra las columnas a castellano.
...
* 📝 Selecciona solo aquellos registros con el pelo (`hair_color`) negro
...
Las funciones nos servirán para encapsular nuestro código.
...
* 📝 **Ejercicio 4**: añadido al filtro anterior, ordena los registros por altura (en orden ascendente) y peso (en orden descendente).
 
* 📝 **Ejercicio 5**: selecciona solo las columnas que contengan variables numéricas, coloca los años de nacimiento como primera columna y cambia los nombres a castellano.
 
* 📝 **Ejercicio 6**: calcula una nueva columna que indique el IMC de cada personaje (define antes una función que lo calcule).

* 📝 **Ejercicio 7**: selecciona los 3 personajes que en más películas han salido y los 3 que menos.
...
La **estadística descriptiva** es la rama de la estadística que se dedica al preprocesamiento, análisis preliminar de las muestras, resumen y visualización.
...
Hemos hablado de **tipos de variables** y algunas medidas como la media. En estadística llamamos **medidas de centralización** a aquellas medidas que nos indican en torno a qué **valores se concentran los datos**

* **Media**: medida de centralización basada en el valor que nos **minimiza el promedio de desviaciones al cuadrado**. La media es el valor que está más «cerca» de todos los puntos a la vez.

```{r}
starwars %>% drop_na(height) %>% summarise(mean(height))
```

**IMPORTANTE**: la media **SOLO se puede calcular para variables cuantitativas**: solo podemos calcular medias de números, no podemos hacer la media de rojo y azul.

---

# Medidas de centralización

* **Mediana**: medida de centralización definida como el **valor que ocupa el centro de los datos** cuando los **ordenamos de menor a mayor**, un valor que nos deja por debajo al menos el 50% y por encima al menos el 50%.

```{r}
starwars %>% drop_na(height) %>%
  summarise(mean(height), median(height))
```

**IMPORTANTE**: la medina **SOLO se puede calcular para variables cuantitativas** o **variables cualitatives ordinales**: solo podemos calcular el valor de en medio en datos que permitan ser ordenados.

---

# Medidas de centralización

* **Moda**: medida de centralización definida como el **valor más repetido**. Al contrario que la media y la mediana, la moda **no tiene porque ser única**: podemos tener un empate. Si todos se repiten por igual, al no destacar ninguno, se dice que es **amodal**.

La ventaja es que se puede **calcular para todo tipo de variables**. El incoveniente es que la moda tenemos algún problema mayor ya que no hay un función en los paquetes `{base}` para su cálculo directo. Para calcularla podemos usar el paquete `{modeest}` la función `mfv()` nos calcula la moda exacta de una variable numérica discreta o cuantitativa, la función `mlv()` nos calcula la moda estimada de una variable numérica continua.

```{r}
library(modeest)
starwars %>% drop_na(height) %>%
  summarise(mean(height), median(height), mlv(height))
```
...
* La **mediana es algo más robusta**: para que cambie en el ejemplo se necesitarían cambiar al menos la mitad de los elementos.

```{r}
median(c(1, 1, 1, 1, 2, 3, 4, 5, 6, 7))
```

* La **moda es la más robusta**: para que cambie en el ejemplo se necesitarían cambiar más de la mitad de los datos Y ADEMÁS que los nuevos sean todos repetidos.
...
starwars %>% drop_na(mass) %>%
  summarise(media = mean(mass), mediana = median(mass),
            var = var(mass), sd = sd(mass),
            p13 = quantile(mass, probs = c(0.13)),
            p87 = quantile(mass, probs = c(0.87)))
```
...
starwars %>% drop_na(mass, height) %>%
  summarise(media = across(c(mass, height), mean),
            var = across(c(mass, height), var),
...
Otra opción con **variables cualitativas**, en especial para visualizar palabras en textos y documentos, son precisamente las **word cloud o nubes de palabras**. Con el paquete `{wordcloud2}` podremos **visualizar las nubes de términos**, por ejemplo de las **especies de starwars**, en un tamaño relativo al número de veces que aparece.
...
## VISUALIZANDO EN COORDENADAS POLARES

&nbsp;

Vamos a imitar a Florence Nightingale
...
El gráfico de rosa o área polar fue una absoluta revolución ya que permitía representar tres variables a la vez: 
...
```{r fig.cap = "Gráfico original de Florence Nightingale", out.width = '50%'}
knitr::include_graphics("img/rosa_nightingale.jpg")
```
...
# Casos prácticos: informes RMarkdown
